...
function displayResults(responseJson) {
  ...
  for (let i = 0; i < responseJson.animals.length; i++){
    html += '<li>'
    const header = `<h1><a href="${responseJson.animals[i].url}">${responseJson.animals[i].name}</a></h1>`
    html += header
    ...
    if (responseJson.animals[i].photos.length) {
      ...
      for (let photo of photos) {
        html += `<img src="${photo.small}" class="banner-img" alt="photo of ${responseJson.animals[i].name}"/>`
      }
    }
    html += '</li>'
  }
